    ...
    TimerT: Clone + TimeSinceEpoch,
{
    ...
    fn apply_native_token_mirror_state_diff(
        ...
        for (address, account) in state_diff.as_inner() {
            if *address != native_token_mirror.token {
                let mirror_balance =
                    native_token_mirror.native_to_erc20_balance(account.info.balance);
                let storage_key = native_token_mirror.balance_storage_key(*address);
                let old_value = state.set_account_storage_slot(
                    native_token_mirror.token,
                    storage_key,
                ...
                let account_info =
                    state
                        ...
                        .and_then(|mut account_info| {
                            ...
                            {
                                account_info.code =
                                    Some(state.code_by_hash(account_info.code_hash)?);
                            }
        ...
        state_diff.apply_diff(mirrored_changes.into());
