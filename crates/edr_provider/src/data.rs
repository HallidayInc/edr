    ...
    TimerT: Clone + TimeSinceEpoch,
{
    ...
    fn apply_native_token_mirror_account_change(
        ...
        let slot = EvmStorageSlot::new_changed(old_value, mirror_balance, TransactionId::ZERO);
        let account_info =
            state
    ...
    TimerT: Clone + TimeSinceEpoch,
{
    ...
    pub fn estimate_gas(
        ...
        self.execute_in_block_context(Some(block_spec), |blockchain, block, state| {
            ...
            let mut result = gas::estimate_gas::<TimerT, ChainSpecT>(
                &context,
                contract_decoder,
                minimum_cost,
                &observer_config,
            ...
            result.estimation =
                gas::adjust_estimation::<ChainSpecT>(result.estimation, context.header.gas_limit);
            Ok(result)
        })?
    }
}
    ...
    let (blockchain, mut irregular_state) =
        tokio::task::block_in_place(|| -> Result<_, ForkedCreationError<ChainSpecT::Hardfork>> {
            ...
            let blockchain = runtime.block_on(
                ForkedBlockchainForChainSpec::<ChainSpecT>::new_with_hardfork_selection(
                    ...
                    config.resolve_hardfork_at_fork,
                ),
            )?;
...
>(
    ...
    if let Some(native_token_mirror) = &native_token_mirror {
        ...
        for (address, native_balance) in mirrored_balances {
            ...
            let storage_key = native_token_mirror.balance_storage_key(address);
            let slot = EvmStorageSlot::new_changed(U256::ZERO, erc20_balance, TransactionId::ZERO);
