...
use crate::{ArbChainSpec, GenericChainSpec};

fn block_env_from_header(header: &BlockHeader, hardfork: EvmSpecId) -> edr_chain_l1::BlockEnv {
...
fn block_env_from_partial_header(
...
macro_rules! impl_generic_chain_spec_family {
    ($chain_spec:ty) => {
        impl ChainHardfork for $chain_spec {
            type Hardfork = edr_chain_l1::Hardfork;
        }

        impl ChainSpec for $chain_spec {
        ...
        impl BlockEnvConstructor<BlockHeader> for $chain_spec {
            fn new_block_env(header: &BlockHeader, hardfork: EvmSpecId) -> Self::BlockEnv {
                block_env_from_header(header, hardfork)
            }
        }

        impl BlockEnvConstructor<PartialHeader> for $chain_spec {
            fn new_block_env(header: &PartialHeader, hardfork: EvmSpecId) -> Self::BlockEnv {
                block_env_from_partial_header(header, hardfork)
            }
        }

        impl EthHeaderConstants for $chain_spec {
            const MIN_ETHASH_DIFFICULTY: u64 = L1ChainSpec::MIN_ETHASH_DIFFICULTY;
        }

        impl GenesisBlockFactory for $chain_spec {
        ...
        impl RuntimeSpec for $chain_spec {
            ...
            fn next_base_fee_per_gas(
                ...
                L1ChainSpec::next_base_fee_per_gas(
                    header,
                    chain_id,
                    hardfork,
                    base_fee_params_overrides,
                )
            }
        ...
        impl<TimerT: Clone + TimeSinceEpoch> ProviderSpec<TimerT> for $chain_spec {
            type PooledTransaction = edr_chain_l1::L1PooledTransaction;
            type TransactionRequest = crate::transaction::Request<$chain_spec>;

            fn cast_halt_reason(
                reason: Self::HaltReason,
            ) -> TransactionFailureReason<Self::HaltReason> {
...
impl_generic_chain_spec_family!(GenericChainSpec);
impl_generic_chain_spec_family!(ArbChainSpec);
...
mod tests {
    ...
    macro_rules! block_env_tests {
        ($module:ident, $chain_spec:ty) => {
            mod $module {
                use super::*;

                #[test]
                fn block_constructor_should_default_excess_blob_gas_for_cancun() {
                    let header = build_block_header(None); // No blob gas information
                    let spec_id = edr_chain_l1::Hardfork::CANCUN;

                    let block = <$chain_spec>::new_block_env(&header, spec_id);
                    assert_eq!(
                ...
                #[test]
                fn block_constructor_should_default_excess_blob_gas_after_cancun() {
                    let header = build_block_header(None); // No blob gas information
                ...
                #[test]
                fn block_constructor_should_not_default_excess_blob_gas_before_cancun() {
                    let header = build_block_header(None); // No blob gas information

                    let block =
                        <$chain_spec>::new_block_env(&header, edr_chain_l1::Hardfork::SHANGHAI);
                    assert_eq!(block.blob_excess_gas_and_price, None);
                }

                #[test]
                fn block_constructor_should_use_existing_excess_blob_gas() {
    ...
    block_env_tests!(generic, GenericChainSpec);
    block_env_tests!(arb, ArbChainSpec);
}
