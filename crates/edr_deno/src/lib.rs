...
fn configured_hardfork<HardforkT>(
    hardfork: Option<&str>,
    chains: Option<&[ChainConfig]>,
    chain_id: u64,
    fork_block_number: Option<u64>,
    parse_spec_id: impl Fn(&str) -> Option<HardforkT>,
) -> Option<HardforkT> {
    if let Some(hardfork) = hardfork.and_then(&parse_spec_id) {
        return Some(hardfork);
    }

    // When no explicit hardfork is provided, use the chain's hardfork history
    // for the fork point. If the fork block is selected remotely during provider
    // creation, only a genesis activation can be selected safely here.
    let fork_block_number = fork_block_number.unwrap_or(0);
    chains
        .and_then(|chains| chains.iter().find(|chain| chain.chain_id == chain_id))
        .and_then(|chain| {
            chain
                .hardforks
                .iter()
                .filter(|activation| activation.block_number <= fork_block_number)
                .max_by_key(|activation| activation.block_number)
        })
...
fn apply_common_provider_options<HardforkT>(
    cfg: &mut edr_provider::config::ProviderConfig<HardforkT>,
...
pub fn provider_new(
    ...
    let entry = match opts.chain {
        Chain::L1 => {
            ...
            apply_common_provider_options(&mut cfg, &opts);
            ...
            if let Some(hardfork) = configured_hardfork(
                opts.hardfork.as_deref(),
                opts.chains.as_deref(),
                cfg.chain_id,
        ...
        Chain::Op => {
            ...
            // `OpSpecId::default()` can point at a hardfork for which EDR only has
            // partial support. Keep the Deno provider's default aligned with the
            // latest hardfork that EDR exposes as supported.
            cfg.hardfork = edr_op::Hardfork::ISTHMUS;
            apply_common_provider_options(&mut cfg, &opts);
            if let Some(id) = opts.chain_id {
                cfg.chain_id = id;
                if opts.network_id.is_none() {
        ...
        Chain::Injective => {
            ...
            match Provider::<InjectiveChainSpec>::new(
                runtime.handle().clone(),
                Box::new(FfiLogger::new(id, log_cb, decode_cb, log_enabled != 0)),
                Box::new(|_event| {}),
                cfg,
                ...
                Ok(p) => ProviderEntry::Injective(Arc::new(p)),
                Err(_) => return 0,
            }
        }
        Chain::Arc => {
...
pub fn provider_handle_request(id: u32, request: &str) -> String {
    ...
    macro_rules! handle_request {
        ($provider:ident, $chain:ty) => {{
            let req: edr_provider::requests::ProviderRequest<$chain> =
                match serde_json::from_str(request) {
                    ...
                    Err(error) => {
                        ...
                        if let Some((name, provider_error)) =
                            reason.provider_error::<$chain, CurrentTime>()
                        {
    ...
    match entry {
        ProviderEntry::L1(provider) => handle_request!(provider, L1ChainSpec),
        ProviderEntry::Op(provider) => handle_request!(provider, OpChainSpec),
        ProviderEntry::Generic(provider) => handle_request!(provider, GenericChainSpec),
        ProviderEntry::Injective(provider) => handle_request!(provider, InjectiveChainSpec),
        ProviderEntry::Arc(provider) => handle_request!(provider, ArcChainSpec),
        ProviderEntry::Arb(provider) => handle_request!(provider, ArbChainSpec),
        ProviderEntry::Ape(provider) => handle_request!(provider, ApeChainSpec),
        ProviderEntry::Tempo(provider) => handle_request!(provider, TempoChainSpec),
    }
}
