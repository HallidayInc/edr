...
fn fork_chain_overrides_mut<HardforkT>(
    network: &mut NetworkConfig<HardforkT>,
) -> Option<&mut HashMap<u64, ChainOverride<HardforkT>>> {
    match network {
        NetworkConfig::Fork(fork_config) => Some(&mut fork_config.chain_overrides),
        NetworkConfig::Local(_) => None,
    }
}
...
pub fn provider_new(
    ...
    let entry = match opts.chain {
        Chain::L1 => {
            ...
            if let Some(chain_overrides) = fork_chain_overrides_mut(&mut cfg.network) {
                if let Some(chains) = &opts.chains
                    && !chains.is_empty()
                {
                    chain_overrides.extend(self::chain_overrides(chains, parse_l1_spec_id));
                }
        ...
        Chain::Op => {
            ...
            if let Some(chain_overrides) = fork_chain_overrides_mut(&mut cfg.network) {
                if let Some(chains) = &opts.chains
                    && !chains.is_empty()
                {
                    chain_overrides.extend(self::chain_overrides(chains, parse_op_spec_id));
                }
                insert_native_token_mirror_override(
                    chain_overrides,
                    chain_id,
        ...
        Chain::Generic => {
            ...
            if let Some(chain_overrides) = fork_chain_overrides_mut(&mut cfg.network) {
                ...
                if !chain_overrides.contains_key(&chain_id)
                    && let Some(acts) = default_l1_activations()
                {
                    insert_default_l1_activations(chain_overrides, chain_id, &acts);
                }
                insert_native_token_mirror_override(
                    chain_overrides,
                    chain_id,
