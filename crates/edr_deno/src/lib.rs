...
use edr_eth::signature::{public_key_to_address, SecretKey, SignatureError};
use edr_eth::hex;
use edr_eth::HashMap;
use edr_eth::{
    l1::{self, L1ChainSpec},
    ...
    let entry = match opts.chain {
        Chain::L1 => {
            let fork = fork_opts.as_ref().map(|f| {
                let mut chain_overrides: HashMap<u64, hardfork::ChainOverride<l1::SpecId>> =
                    HashMap::new();
            ...
            let mut cfg = if fork.is_some() {
                test_utils::create_test_config_with_fork::<l1::SpecId>(fork)
            } else {
        ...
        Chain::Op => {
            let fork = fork_opts.as_ref().map(|f| {
                let mut chain_overrides: HashMap<u64, hardfork::ChainOverride<edr_op::OpSpecId>> =
                    HashMap::new();
                if let Some(chains) = &opts.chains {
                    for c in chains {
                        let mut activations = Vec::new();
            ...
            let mut cfg = if fork.is_some() {
                test_utils::create_test_config_with_fork::<edr_op::OpSpecId>(fork)
            } else {
