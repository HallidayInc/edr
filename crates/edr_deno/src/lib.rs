...
fn default_l1_activations() -> Option<hardfork::Activations<l1::Hardfork>> {
    l1_hardfork::chain_config(1).map(|config| config.hardfork_activations.clone())
}
...
pub fn provider_new(
    ...
    let entry = match opts.chain {
        ...
        Chain::Op => {
            let fork = fork_opts.as_ref().map(|f| {
                let mut chain_overrides: HashMap<u64, hardfork::ChainOverride<edr_op::Hardfork>> =
                    HashMap::new();
                if let Some(chains) = &opts.chains {
                    for c in chains {
                        let mut activations = Vec::new();
