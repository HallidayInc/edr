...
Deno.test("native token mirror preserves unrelated token storage after mining", async () => {
    ...
    const slot = 51n;
    const accountABalance = 11n * 10n ** 18n;
    const accountBBalance = 12n * 10n ** 18n;
    const accountAStorageKey = mappingStorageKey(accountA, slot);
    ...
    await request(provider, {
        method: "hardhat_setBalance",
        params: [accountA, toRpcQuantity(accountABalance)],
    });
    await request(provider, {
        method: "hardhat_setBalance",
        params: [accountB, toRpcQuantity(accountBBalance)],
    });

    const accountAStorageBefore = await request(provider, {
        method: "eth_getStorageAt",
        params: [token, accountAStorageKey, "latest"],
    });
    assertEquals(decodeFirstWord(accountAStorageBefore), 11n * 10n ** 6n);
    ...
    const accountAStorageAfter = await request(provider, {
        method: "eth_getStorageAt",
        params: [token, accountAStorageKey, "latest"],
    });
