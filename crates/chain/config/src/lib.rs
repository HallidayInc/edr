...
impl NativeTokenMirror {
    ...
    /// Returns the scale between native units and ERC-20 units.
    ///
    /// The mirror currently supports ERC-20 decimals less than or equal to the
    /// native token precision.
    pub fn native_to_erc20_scale(&self) -> U256 {
        assert!(
            self.decimals() <= Self::NATIVE_DECIMALS,
            "native token mirror decimals must be <= 18"
        );

        pow10(Self::NATIVE_DECIMALS - self.decimals())
    }
    ...
    pub fn native_to_erc20_balance(&self, balance: U256) -> U256 {
        balance / self.native_to_erc20_scale()
    }

    /// Converts a mirrored ERC-20 balance into the native token balance.
    pub fn erc20_to_native_balance(&self, balance: U256) -> U256 {
        balance.saturating_mul(self.native_to_erc20_scale())
    }

    /// Returns the native-unit remainder that the ERC-20 balance slot cannot
    /// represent.
    pub fn native_remainder(&self, balance: U256) -> U256 {
        balance % self.native_to_erc20_scale()
    }

    /// Combines an ERC-20 balance slot value with a raw native balance whose
    /// low-order units carry the mirror remainder.
    pub fn mirrored_native_balance(&self, erc20_balance: U256, raw_native_balance: U256) -> U256 {
        self.erc20_to_native_balance(erc20_balance)
            .saturating_add(self.native_remainder(raw_native_balance))
    }
}
